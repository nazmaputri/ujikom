                ...
                @endphp
                    @foreach ($users as $index => $user)
                        <tr class="bg-white hover:bg-sky-50 user-row" data-role="{{ $user->role }}">
                            <td class="px-4 py-1 text-center text-gray-600 text-sm">{{ $startNumber + $index }}</td>
                            <td class="px-4 py-1 text-gray-600 text-sm">{{ $user->name }}</td>
                            <td class="px-4 py-1 text-gray-600 text-sm">{{ $user->email }}</td>
                            <td class="py-3 px-6 text-center">
                                <div class="flex items-center justify-center space-x-5">
                                    <!-- Tombol Lihat Detail -->
                                    <a href="{{ route('detaildata-peserta', ['id' => $user->id]) }}" class="text-white bg-sky-300 p-1 rounded-md hover:bg-sky-200">
                                    ...
                                    <div id="deleteModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
                                        <div class="bg-white p-6 rounded-lg shadow-lg">
                                            ...
                                            <div class="mt-4 flex justify-center space-x-4">
                                                <button onclick="closeDeleteModal()" class="px-4 py-2 bg-red-500 hover:bg-red-400 text-white rounded-md">Batal</button>
                                                <button onclick="confirmDelete()" class="ml-2 px-4 py-2 bg-sky-400 hover:bg-sky-300 text-white rounded-md">Hapus</button>
                                            </div>
                                        </div>
                                    </div>
