...
<div class="inline-block h-auto sm:h-[450px] flex">
    ...
    <main class="flex-1 flex flex-col">
        @if ($activeChat)
            ...
            <!-- Pesan Chat -->
            <div class="bg-[url('{{ asset('storage/wp-chat.jpg') }}')] bg-repeat flex-1 overflow-y-auto p-4 bg-neutral-50 scrollbar-hidden">
                    ...
                    <div class="flex items-start mb-4 @if($message->sender_id == auth()->id()) justify-end @else justify-start @endif">
                        ...
                        @if($message->sender_id == auth()->id())
                            <div class="bg-blue-500 text-white p-3 rounded-lg shadow-md">
                                <p>{{ $message->message }}</p>
                                <p class="text-xs text-white mt-1">{{ $message->created_at->diffForHumans() }}</p>
                            </div>
                        <!-- Pesan Student (Blue) -->
                        @else
                            <div class="border text-gray-600 p-3 rounded-lg shadow-md bg-white">
                                <p>{{ $message->message }}</p>
            ...
            <div class="p-4 bg-white border-t border-gray-300">
                <form action="{{ route('chat.send', $activeChat->id) }}" method="POST" class="flex items-center">
                    ...
                    <input type="text" name="message" placeholder="Ketikkan pesan..." 
                           class="w-full text-gray-700 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" required>
